...
#         self.name = os.path.splitext(os.path.basename(path))[0]
        self.name = os.path.basename(os.path.dirname(path))  # yhd,gl
#         self.name = os.path.basename(path)  # qingyi
#         self.name = self.name.split(".")[0]  # qingyi
#         self.name = os.path.basename(path)  # shenzhen-new
#         if '-' in self.name:
#             self.name = self.name.split("-")[0].replace('.svs', '')
#         else:
#             self.name = self.name.replace('.svs', '')
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        patch_downsample = (int(self.level_downsamples[patch_level][0]), int(self.level_downsamples[patch_level][1]))
        # patch 尺寸 patch-level=0:256;patch-level=1:512
        ref_patch_size = (patch_size*patch_downsample[0], patch_size*patch_downsample[1])
        print('patch_size:',ref_patch_size)
        img_w, img_h = self.level_dim[0]
        if use_padding:
            stop_y = start_y+h
            stop_x = start_x+w
